...
<template>
  <UModal
    ...
    :ui="{
      content: 'max-w-4xl',
      body: 'flex-1 flex flex-col min-h-0 overflow-hidden p-6',
      footer: 'justify-end',
    }"
    @update:open="
      (v) => {
  ...
  >
    <template #title>
      <div class="flex items-center gap-2 text-xl">
        ...
        <span>{{ scrapeResult ? "对比 & 选择更新字段" : "音乐刮削" }}</span>
      </div>
    </template>
    ...
    <template #body>
      ...
      <template v-if="!scrapeResult">
        <div class="flex gap-2 mb-4">
          <UButton
            ...
            :key="p.value"
            class="flex-1"
            :color="selectedPlatform === p.value ? 'primary' : 'neutral'"
            :variant="selectedPlatform === p.value ? 'solid' : 'soft'"
            @click="
        ...
        <div class="flex-1 overflow-y-auto space-y-2 min-h-0">
          ...
          <div
            ...
            :key="item.sourceId"
            class="flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors bg-elevated border border-transparent"
            :class="
              isLoadingDetail && scrapeResult === null && results[0]?.sourceId
                ? 'pointer-events-none'
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <p
                ...
                :class="[
                  item.title === existingMusic?.title ? 'text-primary' : '',
                ]"
              >
                {{ item.title }}
              </p>
              ...
              <p
                ...
                class="text-primary text-xs truncate"
              >
                {{ item.cover }}
              </p>
            </div>
      ...
      <template v-else>
        <div class="flex items-center justify-between mb-4 gap-3 flex-wrap">
          ...
          <div class="flex items-center gap-2 text-xs text-zinc-500 ml-auto">
        ...
        <div class="flex-1 overflow-y-auto space-y-3 min-h-0 pr-1">
          ...
          >
            ...
            <!-- 字段名 -->
            <div class="mt-1 flex items-center gap-1.5 shrink-0">
              <component :is="rowIcon(field)" class="w-4 h-4 shrink-0" />
            ...
            <!-- 当前值 -->
            <div
              class="min-w-0 rounded-lg bg-elevated px-3 py-2 text-xs"
              :class="{ 'max-h-40 overflow-y-auto': field === 'lyrics' }"
            >
              <div class="text-[10px] uppercase tracking-wider mb-1">当前</div>
              <!-- 封面 -->
              ...
              <template v-else>
                <div
                  :class="{
                    'wrap-break-word whitespace-pre-wrap': true,
            ...
            >
              <div
                ...
                :class="hasDiff(field) ? 'text-primary' : 'text-muted'"
              >
                刮削
              </div>
              <template v-if="field === 'cover'">
                ...
                <div
                  v-else
                  class="w-16 h-16 rounded bg-accented flex items-center justify-center text-xs"
                >
                  无
                </div>
              </template>
    ...
    <!-- 底部操作（第二阶段） -->
    <template v-if="scrapeResult" #footer>
      <div class="flex items-center justify-end gap-3">
        <UButton color="neutral" variant="soft" @click="backToSearch">
          返回
        </UButton>
        <UButton
          color="primary"
          ...
          @click="confirmSelect"
        >
          确认更新（{{ ALL_FIELDS.filter((f) => selectedFields[f]).length }}
          项）
        </UButton>
      </div>
    </template>
