...
export const getRecordID = (file: IRecordIDExtend) =>
  Check.isString(file) ? file : file.record_id;
...
export class QuarkUCFSApi {
  ...
  sort(param?: IQuerySortParam): Promise<IQueryPageResult<IFile>> {
    throw "";
  }
  info(
    file: IFidExtend,
    param?: { _fetch_full_path?: number },
  ): Promise<IFile> {
    throw "";
  }
  rename(file: IFidExtend, newName: string): Promise<void> {
    throw "";
  }
  mkdir(name: string, param?: IMkderParam): Promise<IFile> {
    throw "";
  }
  delete(file: IFidExtend[], exclude?: IFileFid[]): Promise<ICreateTaskResult> {
    throw "";
  }
   ...
   *
   */
  preUpdate(param: IPreUpdateParam): Promise<IPreUpdateResult> {
    throw "";
  }
  hashUpdate(param: IHashUpdateParam): Promise<IHashUpdateResult> {
    throw "";
  }
  finishUpdate(param: IFinishUpdateParam): Promise<IFinishUpdateResult> {
    throw "";
  }
  partUpdate(
    pre: IPartUpdateParam,
    partNumber: number,
    data: Uint8Array,
  ): Promise<string> {
    throw "";
  }
  upCommit(pre: IPartUpdateParam, md5s: string[]) {
    throw "";
  }
  task<E>(taskID: string, _await?: boolean): Promise<ITaskStateResult<E>> {
    throw "";
  }
  ...
  /** 回收站 */
  deepRecycleList(
    param?: IQueryRecycleListParam,
  ): Promise<IQueryRecycleListResult> {
    throw "";
  }
  /** 回收站 */
  recycleList(
  ...
  /** 清空回收站 */
  recycleClear(): Promise<void> {
    throw "";
  }
  /** 删除指定回收站文件 */
  recycleRemove(file: IRecordIDExtend | IRecordIDExtend[]): Promise<void> {
    throw "";
  }
  /** 恢复回收站文件 */
  recycleRecover(
    file: IRecordIDExtend | IRecordIDExtend[],
  ): Promise<ICreateTaskResult> {
    throw "";
  }
}

export type IQuerySortParam = Partial<
  IQueryPageParam<
    {
      pdir_fid: string;
      force: NBoolean;
...
QuarkUCFSApi.prototype.sort = async function (params = {}) {
  ...
  } = await this.client.agentApi.get("/file/sort").query({
    ...params,
    _fetch_total: 1,
  });
...
QuarkUCFSApi.prototype.info = async function (file, param = {}) {
  ...
  const {
    body: { data },
  } = await this.client.agentApi.get("/file/info").query({ fid, ...param });
  return data;
};
...
QuarkUCFSApi.prototype.mkdir = async function (
  file_name: string,
  params: IMkderParam = {},
) {
  const {
    body: { data },
  } = await this.client.agentApi.post("/file").send({
    file_name,
    pdir_fid: "0",
    dir_init_lock: false,
    dir_path: "",
...
QuarkUCFSApi.prototype.rename = async function (file, file_name) {
  const fid = getFileFid(file);
  await this.client.agentApi.post("/file/rename").send({ fid, file_name });
};
...
QuarkUCFSApi.prototype.download = async function (files) {
  ...
  const {
    body: { data },
  } = await this.client.agentApi.post("/file/download").send({ fids });
...
QuarkUCFSApi.prototype.preUpdate = async function (param) {
  ...
  const {
    body: { data },
  } = await this.client.agentApi.post("/file/upload/pre").send({
...
QuarkUCFSApi.prototype.finishUpdate = async function (param) {
  const {
    body: { data },
  } = await this.client.agentApi.post("/file/update/finish").send(param);
  return data;
};

export type IPartUpdateParam = Pick<
  IPreUpdateResult,
  | "format_type"
  | "upload_id"
  | "bucket"
...
/${bucket}/${obj_key}?partNumber=${partNumber}&uploadId=${upload_id}`;
  ...
  const {
    ...
    .send({ auth_info, auth_meta, task_id });

  const result = await this.client.agent
    .put(`https://${bucket}.${upload_url.substring(7)}/${obj_key}`)
    .set({
  ...
  if (result.status != 200)
    throw HttpError.create(
      `up status: ${result.status}, error: ${result.text}`,
    );
...
/${bucket}/${obj_key}?uploadId=${upload_id}`;
  ...
  const result = await this.client.agent
    .post(`https://${bucket}.${upload_url.substring(7)}/${obj_key}`)
    .set({
      Authorization: auth_key,
...
export type IQueryRecycleListParam = Partial<
  IQueryPageParam<{}, "move_recycle_at">
>;
...
QuarkUCFSApi.prototype.recycleRecover = async function (files) {
  ...
  const {
    body: { data },
  } = await this.client.agentApi.post("/file/recycle/recover").send({
    select_mode: 2,
    record_list,
  });
  return data;
...
QuarkUCFSApi.prototype.recycleRemove = async function (files) {
  ...
  const {
    body: { data },
  } = await this.client.agentApi.post("/file/recycle/remove").send({
    select_mode: 2,
    record_list,
  });
  return data;
