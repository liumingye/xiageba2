...
export class BaiduShareFSApi {
  ...
  wxlist(param: IListShareParam): Promise<IListShareResult> {
    throw "";
  }

  transfer(
    param: ITransferShareParam,
    savePath: string,
    ...fsid: StringNumber[]
  ): Promise<ITransferShareResult> {
    throw "";
  }

  sharedownload(param: IShareDownloadParam): Promise<IShareDownloadResult> {
    throw "";
  }
...
export type IShareParam = { pwd?: string } & (
  | { shorturl: string }
  | { shareid: StringNumber; uk: StringNumber }
);
export type IListShareParam = IListParam<
  IShareParam & {
    root?: NBoolean;
  }
>;
...
BaiduShareFSApi.prototype.wxlist = async function (params) {
  const {
    body: { data },
  } = await this.request(Method.POST, "/share/wxlist")
...
BaiduShareFSApi.prototype.sharedownload = async function (
  param: IShareDownloadParam,
) {
  if (
    typeof this.client.source !== "string" ||
    !this.client.bduss ||
    !this.client.uid ||
    !this.client.androidChannel.sk
  ) {
    throw new Error(
      "sharedownload is only supported when using cookie-based authentication and user details (uid, sk) are available.",
    );
  }
  ...
  const post_body_string = post_data_items
    .map(([k, v]) => `${k}=${v}`)
    .join("&");

  const sign = generateSharedownloadSign(
    post_body_string,
    Android.DEVICE_ID,
    timestamp,
  );
...
BaiduShareFSApi.prototype.createShare = async function ({
  fsidList,
  pwd,
  period = 0,
}) {
  const fid_list = JSON.stringify(fsidList.map((v) => parseInt(v as any)));
  const { body } = await this.request(Method.POST, "/share/set")
    .type(ContentType.FormUrlencoded)
    .query({
      channel: "chunlei",
