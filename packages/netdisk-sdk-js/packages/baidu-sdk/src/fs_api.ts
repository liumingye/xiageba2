...
export class BaiduFSApi {
  ...
  gettemplatevariable<T extends string>(k: T[]): Promise<Record<T, unknown>> {
    throw "";
  }

  /** 下载地址获取 */
  filemetas(param: IFileMetasParam, ua?: string): Promise<IFileMetasResult> {
    throw "";
  }
  list(param: IListParam): Promise<IListResult> {
    throw "";
  }
  /** 查询文件列表 */
  listall(param: IListParam<IListAllExtend>): Promise<IListResult> {
    throw "";
  }
  /** 管理文件 */
  filemanager<Op extends IFileManagerOpera>(
    opera: Op,
    param: IFileManagerParam<IFileManagerFileListTypeMap[Op]>,
  ): Promise<IFileManagerResult> {
    throw "";
  }
   ...
   ***********/
  /** 预上传 */
  precreate(param: IPrecreateFileParam): Promise<IPrecreateFileResult> {
    throw "";
  }
  /** 秒传  */
  // rapidupload(){ throw '' }
  /** 上传完毕创建文件 */
  create(param: ICreateFileParam | ICreateDirParam): Promise<IFile> {
    throw "";
  }
  /** 上传分片 */
  updatePart(
    param: IUpdateParam,
    file: Blob | NodeJS.BufferSource | ReadableStream,
   ...
   * 回收站
   ********/
  recycleList(param: IRecycleListParam): Promise<IRecycleListResult> {
    throw "";
  }
  recycleDelete(
    param?: IRecycleParam,
  ...
  recycleClear(param?: IRecycleParam): Promise<IRecycleResult> {
    throw "";
  }
  recycleRestore(
    param?: IRecycleParam,
...
BaiduFSApi.prototype.gettemplatevariable = async function (fields) {
  const {
    body: { result },
  } = await this.request(Method.GET, "/api/gettemplatevariable").query({
...
export type IListParam<Extend = {}, Page = IPageParam & IOffsetParam> = Extend &
  Page & {
    dir: string;
    /** 默认为文件类型 */
...
BaiduFSApi.prototype.list = async function (param) {
  const { body } = await this.request(Method.GET, "/api/list").query({
    ...param,
  });
  return body;
};
BaiduFSApi.prototype.listall = async function (param) {
  const { body } = await this.request(Method.GET, "/api/listall").query({
    ...param,
  });
  return body;
};
...
BaiduFSApi.prototype.updatePart = async function (
  param,
  file,
  progress,
...
BaiduFSApi.prototype.create = async function (param) {
  const block_list =
    "block_list" in param ? JSON.stringify(param.block_list || []) : void 0;
...
BaiduFSApi.prototype.recycleClear = async function (param) {
  const { body } = await this.request(Method.POST, "/api/recycle/clear").query(
    param || {},
  );
  return body;
};
