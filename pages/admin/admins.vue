...
import { Music, Users, LogOut, Plus, Trash2, User, Edit3 } from 'lucide-vue-next'
...
const editAdmin = async () => {
  ...
  if (editUsername.value.trim()) updateData.username = editUsername.value.trim()
  if (editPassword.value.trim()) updateData.password = editPassword.value.trim()
  ...
  <div class="min-h-screen bg-dark-300">
    ...
    <nav class="bg-gray-900/50 border-b border-gray-800 px-6 py-3">
      <div class="flex items-center gap-4 max-w-7xl mx-auto">
        <a href="/admin" class="flex items-center gap-2 text-gray-400 hover:text-gray-200 transition-colors">
          <Music class="w-5 h-5" />
          音乐管理
        </a>
        <a href="/admin/admins" class="flex items-center gap-2 text-primary-500 font-medium">
          <Users class="w-5 h-5" />
          管理员管理
        </a>
      </div>
      ...
      <div class="card overflow-hidden">
        <table class="w-full">
          <thead class="bg-gray-800">
            <tr>
              <th class="px-4 py-3 text-left text-gray-400 text-sm font-medium">用户名</th>
              <th class="px-4 py-3 text-left text-gray-400 text-sm font-medium">创建时间</th>
              <th class="px-4 py-3 text-center text-gray-400 text-sm font-medium">操作</th>
            </tr>
          </thead>
          <tbody>
            <tr 
        ...
        >
          ...
          <div class="relative bg-gray-900 rounded-xl p-6 max-w-md w-full border border-gray-800">
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-gray-400 text-sm mb-2">密码（留空则不修改）</label>
                <input
