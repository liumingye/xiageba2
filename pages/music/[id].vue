...
import { ArrowLeft, Download, Play, Pause, Disc3 } from 'lucide-vue-next'
  ...
  <div class="min-h-screen bg-dark-300 py-6 px-4">
    ...
    <main class="max-w-3xl mx-auto">
      <div v-if="loading" class="flex justify-center py-20">
        <div class="w-8 h-8 border-4 border-primary-500 border-t-transparent rounded-full animate-spin" role="status" aria-label="加载中"></div>
      </div>

      <article v-else-if="music" class="space-y-6" itemscope itemtype="https://schema.org/MusicRecording">
        ...
        <section class="card p-6">
          <div class="flex flex-col md:flex-row gap-6">
            <div class="relative flex-shrink-0">
              <img
                :src="music.cover || 'https://via.placeholder.com/200'"
                :alt="music.title"
                class="w-48 h-48 md:w-64 md:h-64 rounded-xl object-cover"
                loading="lazy"
              />
              <div
            ...
            <div class="flex-1 flex flex-col justify-center">
              <h1 class="text-2xl md:text-3xl font-bold text-white mb-2">{{ music.title }}</h1>
              <p class="text-gray-400 mb-4" itemprop="byArtist">{{ music.artist }}</p>
        ...
        <section class="card p-6" itemscope itemtype="https://schema.org/MusicAlbum">
          <div class="flex items-center gap-2 text-gray-400 mb-4">
            <Disc3 class="w-5 h-5" />
            <span>所属专辑</span>
          </div>
          <p class="text-white text-lg" itemprop="name">{{ music.album || '未填写' }}</p>
        </section>

        <section class="card p-6">
          <h3 class="text-lg font-medium text-white mb-4">歌词</h3>
          <div v-if="formattedLyrics.length > 0" class="space-y-2 text-gray-300" itemprop="lyrics">
        ...
        <section class="card p-6" v-if="music.playUrl">
          <h3 class="text-lg font-medium text-white mb-3">播放信息</h3>
          <p class="text-gray-400 text-sm break-all">{{ music.playUrl }}</p>
        </section>
      </article>
